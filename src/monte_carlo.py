...
class MonteCarloResults:
    trials: int
    days: int
    informed_matrix: np.ndarray      # shape: (trials, days+1)
    ever_reached_matrix: np.ndarray  # shape: (trials, days+1)
...
def print_mc_summary(mc: MonteCarloResults):
    ...
    det_peak  = max(s.informed for s in det)
    det_final = det[-1].pct_reached

    print("\n" + "="*65)
    print("  INFO DIFFUSION — MONTE CARLO ANALYSIS (v0.2)")
    print(f"  {mc.trials} trials  |  pop {N:,}  |  "
          f"seed {mc.params.seed_viewers:,} viewers  |  R\u2080={mc.params.R0:.2f}")
    print("="*65)
    ...
    valid = d50[~np.isnan(d50)]
    if len(valid):
        print(f"  {'Days to 50% reached':<30} {valid.mean():>10.1f} "
              f"{valid.std():>10.1f} {valid.min():>10.0f} {valid.max():>10.0f}")
    else:
        print(f"  {'Days to 50% reached':<30} {'never':>10}")
    print(f"\n  Deterministic baseline — peak: {det_peak:,}  |  "
          f"ever reached: {det_final:.1f}%")
    print(f"\n  Percentile spread of % ever reached:")
    for p in [5, 25, 50, 75, 95]:
        bar = "█" * int(np.percentile(final, p) / 2)
